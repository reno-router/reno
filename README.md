...
[![deno doc](https://doc.deno.land/badge.svg)](https://doc.deno.land/https/deno.land/x/reno@v1.3.5/reno/mod.ts)
...
```ts
import { jsonResponse, assertResponsesAreEqual } from "https://deno.land/x/reno@v1.3.5/reno/mod.ts";
...
```ts
import { createRouteMap, jsonResponse, pipe } from "https://deno.land/x/reno@v1.3.5/reno/mod.ts";
...
As well as the [example app found in this repo](https://github.com/reno-router/reno/tree/v1.3.5/example), which is targetted by the end-to-end test suite, there is a [standalone repository for a blog microservice](https://github.com/reno-router/blog-microservice) built with Deno, Reno, PostgreSQL, and Docker.

## API Documentation

Consult [Reno's entry on the Deno Doc website](https://doc.deno.land/https/deno.land/x/reno@v1.3.5/reno/mod.ts) for comprehensive documentation on Reno's API.
